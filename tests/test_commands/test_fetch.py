...
def test_fetch_should_add_remote_and_fetch():
    ...
    with patch.object(M, 'getoutput') as mock_getoutput, patch.object(
        M, 'check_call'
    ) as mock_check_call:
        mock_getoutput.return_value = """\
origin  http://github.com/codecli.git (fetch)
origin  http://github.com/codecli.git (push)
"""
        M.main(args)
    ...
    mock_check_call.assert_any_call(
        [
            ...
            'add',
            'testuser',
            'http://github.com/testuser/codecli.git',
        ]
    )
